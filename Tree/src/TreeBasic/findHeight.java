...
public class findHeight {
	...
	public static int heightoftheTree(Tree<Integer> root) {
		int ans=0;
		for (int i=0;i< root.children.size();i++) {
			int height= heightoftheTree(root.children.get(i));
			if (height > ans) {
				ans = height;
			}
	...
	public static void main(String[] args) {
		Tree<Integer> h= takeInputLevelWise();
		int height= heightoftheTree(h);
